...
function AcceptInviteContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (password.length < 6) {
      setError(t("teamAccept.passwordMin"));
      return;
    }
    if (password !== confirm) {
      setError(t("teamAccept.mismatchError"));
      return;
    }
    ...
    try {
      ...
      if (result?.ok) {
        ...
        setError(t("teamAccept.fallbackError"));
        setSuccess(false);
      }
    } catch {
      setError(t("teamAccept.connectionError"));
    } finally {
      setLoading(false);
    }
  };
  ...
  if (success) {
    return (
      <div className="py-8 text-center">
        ...
        <h2 className="text-2xl font-black text-emerald-700">{t("teamAccept.successTitle")}</h2>
        <p className="mt-2 text-slate-500">{t("teamAccept.successDesc")}</p>
  ...
  if (!token) {
    return (
      <div className="py-8 text-center">
        ...
        <h2 className="text-xl font-bold text-red-600">{t("teamAccept.invalidTitle")}</h2>
        <Link href="/login" className="mt-4 inline-block text-sm font-bold text-blue-600 hover:underline">
          {t("teamAccept.backToLogin")}
        </Link>
      </div>
    );
  }
  ...
  return (
    <>
      <div className="mb-8 text-center">
        ...
        <h1 className="mt-2 text-3xl font-black text-slate-950">{t("teamAccept.title")}</h1>
        <p className="mt-2 text-slate-500">{t("teamAccept.subtitle")}</p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-5" noValidate>
        <div>
          <label className="mb-2 block text-sm font-bold text-slate-600">
            {t("teamAccept.newPassword")}
          </label>
          <input
            type="password"
            value={password}
