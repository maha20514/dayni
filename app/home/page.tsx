...
function DashboardMockup() {
  ...
  const customers = [
    { name: "أحمد محمد", nameEn: "Ahmed Mohammed", amount: "350", paid: false },
    { name: "خالد علي",   nameEn: "Khalid Ali",     amount: "120", paid: true },
    { name: "سعد العمري", nameEn: "Saad Al-Omari",  amount: "780", paid: false },
  ];
  ...
  return (
    <div className="relative rounded-2xl sm:rounded-3xl border border-slate-200 bg-white p-4 sm:p-5 shadow-2xl">
      <div className="mb-4 sm:mb-5 flex items-center justify-between border-b border-slate-100 pb-3 sm:pb-4">
        <h2 className="text-sm sm:text-base font-bold text-slate-900">{t("home.dashboardTitle")}</h2>
        <span className="rounded-lg sm:rounded-xl bg-blue-50 px-2.5 sm:px-3 py-1 text-xs sm:text-sm font-black text-blue-700">{t("common.appName")}</span>
      </div>

      {/* stats */}
      <div className="grid grid-cols-3 gap-2 sm:gap-3">
        <div className="rounded-xl sm:rounded-2xl bg-red-50 p-2.5 sm:p-4">
          <p className="text-[10px] sm:text-xs font-bold text-red-500">{t("home.debt")}</p>
          <h3 className="mt-0.5 sm:mt-1 text-sm sm:text-lg font-black text-red-700">{locale === "ar" ? "4,250 ر" : "SAR 4,250"}</h3>
        </div>
        <div className="rounded-xl sm:rounded-2xl bg-emerald-50 p-2.5 sm:p-4">
          <p className="text-[10px] sm:text-xs font-bold text-emerald-500">{t("home.paid")}</p>
          <h3 className="mt-0.5 sm:mt-1 text-sm sm:text-lg font-black text-emerald-700">{locale === "ar" ? "1,800 ر" : "SAR 1,800"}</h3>
        </div>
        <div className="rounded-xl sm:rounded-2xl bg-blue-50 p-2.5 sm:p-4">
          <p className="text-[10px] sm:text-xs font-bold text-blue-500">{t("home.customers")}</p>
      ...
      <div className="mt-3 sm:mt-4 rounded-xl sm:rounded-2xl bg-slate-50 p-2.5 sm:p-3">
        <div className="mb-1.5 sm:mb-2 flex justify-between text-[10px] sm:text-xs font-semibold">
          <span className="text-slate-500">{t("home.collectionRate")}</span>
      ...
      <div className="mt-3 sm:mt-4 space-y-1.5 sm:space-y-2">
        {customers.map((c) => {
          const name = locale === "ar" ? c.name : c.nameEn;
          const amount = locale === "ar" ? `${c.amount} ر` : `SAR ${c.amount}`;
          return (
            <div key={name} className="flex items-center justify-between rounded-lg sm:rounded-xl bg-slate-50 px-2.5 sm:px-3 py-2 sm:py-2.5">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-[10px] sm:text-sm font-bold text-slate-900">{name}</p>
                  <p className="text-[9px] sm:text-xs text-slate-400">{amount}</p>
                </div>
              </div>
              <span className={`rounded-full px-1.5 sm:px-2.5 py-0.5 text-[9px] sm:text-xs font-bold ${c.paid ? "bg-emerald-100 text-emerald-700" : "bg-red-100 text-red-700"}`}>
                {c.paid ? t("home.statusPaid") : t("home.statusDue")}
              </span>
            </div>
          );
        })}
      ...
      <div className="mt-3 sm:mt-4 rounded-xl sm:rounded-2xl bg-slate-900 p-3 sm:p-4 text-white">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] sm:text-xs text-slate-400">{t("home.remaining")}</p>
            <h3 className="mt-0.5 sm:mt-1 text-lg sm:text-2xl font-black">{locale === "ar" ? "2,450 ر" : "SAR 2,450"}</h3>
          </div>
      ...
      {/* floating badge */}
      <div className="absolute -start-3 -top-3 sm:-start-4 sm:-top-4 rounded-xl sm:rounded-2xl bg-emerald-500 px-2 sm:px-3 py-1.5 sm:py-2 text-[10px] sm:text-xs font-black text-white shadow-lg shadow-emerald-500/30">
        {t("home.autoUpdated")}
      </div>
    </div>
  );
...
export default function HomePage() {
  ...
  const steps = [
    { num: "١", numEn: "1", title: t("home.step1Title"), desc: t("home.step1Desc"), icon: "✨" },
    { num: "٢", numEn: "2", title: t("home.step2Title"), desc: t("home.step2Desc"), icon: "👥" },
    { num: "٣", numEn: "3", title: t("home.step3Title"), desc: t("home.step3Desc"), icon: "📊" },
  ];
    ...
    <main className="min-h-screen overflow-hidden bg-slate-50 text-slate-900">
      ...
      <section className="relative min-h-screen bg-gradient-to-br from-slate-50 via-white to-blue-50">
        <div className="absolute inset-0 pointer-events-none">
          <div className="absolute end-0 top-0 h-72 w-72 sm:h-96 sm:w-96 rounded-full bg-blue-200/40 blur-3xl" />
          <div className="absolute bottom-0 start-0 h-56 w-56 sm:h-80 sm:w-80 rounded-full bg-emerald-200/30 blur-3xl" />
        </div>
        ...
        <div className="container relative z-10 mx-auto grid items-center gap-10 sm:gap-16 px-4 sm:px-6 py-16 sm:py-24 lg:grid-cols-2">
          ...
          <div className="text-center lg:text-start order-2 lg:order-1">
            <div className="mb-6 sm:mb-8 inline-flex items-center gap-2 sm:gap-3 rounded-full border border-slate-100 bg-white px-4 sm:px-5 py-2 sm:py-3 shadow-sm">
              ...
              <span className="text-xs sm:text-sm font-semibold text-slate-700">{t("home.badge")}</span>
            </div>
            ...
            <p className="mt-4 sm:mt-6 text-xs sm:text-sm text-slate-500">{t("home.noCardNote")}</p>
          </div>
      ...
      <section className="bg-white py-16 sm:py-24">
        <div className="container mx-auto px-4 sm:px-6">
          <div className="mx-auto mb-10 sm:mb-16 max-w-2xl text-center">
            <span className="mb-3 sm:mb-5 inline-flex rounded-full bg-blue-50 px-3 sm:px-5 py-1.5 sm:py-2 text-xs sm:text-sm font-bold text-blue-700">{t("home.howItWorksBadge")}</span>
            <h2 className="text-2xl sm:text-4xl font-semibold text-slate-950 md:text-5xl">{t("home.howItWorksTitle")}</h2>
          </div>

          <div className="relative grid gap-6 sm:gap-8 md:grid-cols-3">
            <div className="absolute start-[16.67%] top-10 hidden h-0.5 w-2/3 bg-gradient-to-l from-blue-200 to-emerald-200 md:block" />
            {steps.map((step, i) => (
              <div key={step.num} className="relative text-center">
                ...
                <div className={`absolute start-1/2 top-0 flex h-6 w-6 sm:h-7 sm:w-7 -translate-y-2 translate-x-1/2 items-center justify-center rounded-full text-[10px] sm:text-xs font-black text-white ${i === 0 ? "bg-blue-400" : i === 1 ? "bg-indigo-400" : "bg-emerald-400"}`}>
                  {step.num}
                </div>
      ...
      <section className="bg-slate-50 py-16 sm:py-24">
        <div className="container mx-auto px-4 sm:px-6">
          <div className="mx-auto mb-10 sm:mb-16 max-w-3xl text-center">
            <span className="mb-3 sm:mb-5 inline-flex rounded-full bg-slate-100 px-3 sm:px-5 py-1.5 sm:py-2 text-xs sm:text-sm font-semibold text-slate-700">{t("home.featuresBadge")}</span>
            <h2 className="mb-3 sm:mb-5 text-2xl sm:text-4xl font-semibold leading-tight text-slate-950 md:text-5xl">{t("home.featuresTitle")}</h2>
            <p className="text-sm sm:text-lg leading-relaxed text-slate-600">{t("home.featuresDesc")}</p>
          </div>
      ...
      <section className="relative overflow-hidden bg-slate-50 py-16 sm:py-28">
        ...
        <div className="absolute end-10 top-20 h-48 w-48 sm:h-64 sm:w-64 rounded-full bg-blue-100/70 blur-3xl" />
        <div className="absolute bottom-10 start-10 h-48 w-48 sm:h-64 sm:w-64 rounded-full bg-emerald-100/70 blur-3xl" />
            ...
            <div className="grid lg:grid-cols-[0.9fr_1.1fr]">
              ...
              <div className="relative hidden overflow-hidden bg-gradient-to-br from-blue-600 to-indigo-700 p-8 lg:p-10 text-white lg:block">
                ...
                <div className="relative h-full min-h-[380px]">
                  <div className="absolute end-0 top-6 w-64 lg:w-72 rounded-2xl bg-white/15 p-4 lg:p-5 ring-1 ring-white/20 backdrop-blur-xl">
                    <p className="text-sm font-semibold text-blue-100">{t("home.step1Card")}</p>
                    <h3 className="mt-1.5 text-lg lg:text-2xl font-semibold">{t("home.addCustomerCard")}</h3>
                  </div>
                  <div className="absolute start-4 top-36 w-64 lg:w-72 rounded-2xl bg-white p-4 lg:p-5 text-slate-900 shadow-2xl">
                    <p className="text-sm font-semibold text-slate-500">{t("home.step2Card")}</p>
                    <h3 className="mt-1.5 text-lg lg:text-2xl font-semibold">{t("home.trackBalanceCard")}</h3>
                    <div className="mt-3 rounded-xl bg-red-50 p-3 text-red-700">
                      <div className="flex justify-between font-semibold text-sm"><span>{t("home.remainingLabel")}</span><span>2,450</span></div>
                    </div>
                  </div>
                  <div className="absolute bottom-8 end-12 w-64 lg:w-72 rounded-2xl bg-white/15 p-4 lg:p-5 ring-1 ring-white/20 backdrop-blur-xl">
                    <p className="text-sm font-semibold text-emerald-100">{t("home.step3Card")}</p>
                    <h3 className="mt-1.5 text-lg lg:text-2xl font-semibold">{t("home.recordPaymentCard")}</h3>
                  </div>
                </div>
              </div>

              {/* content side */}
              <div className="p-6 sm:p-8 text-center md:p-10 lg:p-12 lg:text-start">
                <span className="mb-4 sm:mb-6 inline-flex items-center gap-2 rounded-full bg-emerald-50 px-4 sm:px-5 py-2 text-xs sm:text-sm font-semibold text-emerald-700">{t("home.ctaBadge")}</span>
                <h2 className="text-2xl sm:text-4xl font-semibold leading-tight text-slate-950 md:text-5xl">
                  ...
                  <span className="mt-1 sm:mt-2 block">
                    {t("home.ctaTitle2")}{" "}
                    <span className="bg-gradient-to-l from-blue-600 to-emerald-500 bg-clip-text font-black tracking-tight text-transparent">{t("common.appName")}</span>
                    {" "}{t("home.ctaTitle3")}
                  </span>
                </h2>
                ...
                <div className="mt-6 sm:mt-8 grid gap-2 sm:gap-3 grid-cols-1 sm:grid-cols-2">
                  {[t("home.ctaItem1"), t("home.ctaItem2"), t("home.ctaItem3"), t("home.ctaItem4")].map((item) => (
                ...
                <p className="mt-6 sm:mt-8 text-[10px] sm:text-sm font-semibold text-slate-500">{t("home.ctaFooterNote")}</p>
              </div>
            </div>
          </div>
        </div>
