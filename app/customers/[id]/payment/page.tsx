...
export default function CreatePaymentPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!amount || Number(amount) <= 0) { setError(t("paymentPage.invalidAmount")); return; }
    setError("");
    setLoading(true);
    try {
      ...
      if (res.ok) { toast.success(t("paymentPage.success")); router.push(`/customers/${customerId}`); }
      else setError(data.error || t("paymentPage.failed"));
    } catch {
      setError(t("paymentPage.connectionError"));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <main dir={dir} className="relative min-h-screen overflow-hidden bg-slate-50 py-6 sm:py-10 text-slate-900">
      <div className="absolute end-0 top-20 h-56 w-56 sm:h-72 sm:w-72 rounded-full bg-emerald-100/60 blur-3xl" />
      <div className="absolute bottom-20 start-0 h-56 w-56 sm:h-72 sm:w-72 rounded-full bg-blue-100/40 blur-3xl" />
      ...
      <div className="container relative z-10 mx-auto max-w-6xl px-3 sm:px-6">
        ...
        <section className="mb-5 sm:mb-8 overflow-hidden rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-white shadow-lg sm:shadow-xl shadow-slate-200/70">
          <div className="relative p-4 sm:p-8 md:p-10">
            <div className="absolute start-0 top-0 h-20 w-20 sm:h-32 sm:w-32 rounded-ee-[2rem] sm:rounded-ee-[4rem] bg-emerald-50" />
            <div className="absolute bottom-0 end-0 h-20 w-20 sm:h-32 sm:w-32 rounded-ss-[2rem] sm:rounded-ss-[4rem] bg-blue-50" />
            <div className="relative flex flex-col gap-4 sm:gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                <span className="mb-2 sm:mb-4 inline-flex rounded-full bg-emerald-50 px-3 sm:px-5 py-1.5 sm:py-2 text-xs sm:text-sm font-bold text-emerald-700">{t("paymentPage.badge")}</span>
              ...
              >
                {t("paymentPage.backToCustomer")} <span className="ms-1 sm:ms-2">←</span>
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-5 sm:gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="hidden overflow-hidden rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-gradient-to-br from-emerald-500 to-teal-600 p-8 text-white shadow-xl lg:block">
            <div className="relative min-h-[400px]">
              ...
              <div className="absolute -bottom-20 -start-20 h-72 w-72 rounded-full bg-blue-300/20 blur-3xl" />
              <div className="relative">
                ...
                <h2 className="text-2xl lg:text-3xl font-semibold leading-tight">{t("paymentPage.sideTitle")}</h2>
                <p className="mt-4 max-w-md leading-relaxed text-emerald-100">
                  {t("paymentPage.sideDesc")}
                </p>
              </div>
              <div className="absolute bottom-0 end-0 start-0 space-y-3">
                <div className="rounded-2xl bg-white/15 p-4 ring-1 ring-white/20 backdrop-blur">
                  <p className="text-sm font-semibold text-emerald-100">{t("paymentPage.autoStep")}</p>
                  <p className="mt-1.5 text-xl font-semibold">{t("paymentPage.autoStepDesc")}</p>
                </div>
                <div className="rounded-2xl bg-white p-4 text-slate-900 shadow-2xl">
                  <p className="text-sm font-semibold text-slate-500">{t("paymentPage.afterSaveStep")}</p>
                  <p className="mt-1.5 text-xl font-semibold">{t("paymentPage.afterSaveStepDesc")}</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-white p-4 sm:p-6 md:p-8 shadow-lg sm:shadow-xl shadow-slate-200/70">
            <div className="mb-5 sm:mb-8 text-center">
              ...
              <h2 className="text-2xl sm:text-3xl font-semibold text-slate-950">{t("paymentPage.formTitle")}</h2>
              <p className="mt-1 sm:mt-2 text-xs sm:text-sm text-slate-500">{t("paymentPage.formSubtitle")}</p>
            </div>

            <form onSubmit={handleSubmit} className="space-y-4 sm:space-y-6">
              <div>
                <label className="mb-1.5 sm:mb-2 block text-xs sm:text-sm font-bold text-slate-600">{t("paymentPage.amountLabel")}</label>
                <div className="relative">
                  <input
              ...
              {/* Quick amounts */}
              <div>
                <p className="mb-2 text-xs font-bold text-slate-400">{t("paymentPage.quickAmounts")}</p>
              ...
              {amount && Number(amount) > 0 && (
                <div className="rounded-xl sm:rounded-2xl border border-emerald-200 bg-emerald-50 p-3 sm:p-5 text-center">
                  <p className="text-xs sm:text-sm font-bold text-emerald-600">{t("paymentPage.previewWillRecord")}</p>
                  <p className="mt-1 text-2xl sm:text-3xl font-black text-emerald-700">
                    {formatNumber(Number(amount), locale)} {t("customers.riyal")}
                  </p>
                  <p className="mt-1 text-[10px] sm:text-xs text-emerald-500">{t("paymentPage.previewAsPayment")}</p>
                </div>
              )}
              ...
              >
                {loading
                  ? <><span className="h-4 w-4 animate-spin rounded-full border-2 border-white/30 border-t-white" /> {t("paymentPage.submitting")}</>
                  : <><span>✅</span> {t("paymentPage.submit")}</>
                }
              </button>
            </form>
          </div>
