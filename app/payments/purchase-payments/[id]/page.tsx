...
export default function PurchasePaymentPrintPage() {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const fileName = `purchase-payment-${purchasePayment._id?.slice(-6)}.pdf`;
      pdf.save(fileName);

      const blob = pdf.output("blob");
      if (navigator.canShare?.({ files: [new File([blob], fileName, { type: "application/pdf" })] })) {
        await navigator.share({ title: t("print.payerSignature2"), files: [new File([blob], fileName, { type: "application/pdf" })] });
      }
        ...
        <div id="payment-card" className="overflow-hidden rounded-2xl shadow-slate-300/50 bg-white shadow-2xl print:rounded-none print:shadow-none">
          ...
          <div className="p-5 sm:p-8">
            ...
            <div className="flex items-start justify-between border-b gap-3 border-slate-100 p-3 sm:p-5">
              <div className="flex items-center gap-3 sm:gap-4">
                ...
                <div>
                  ...
                  <p className="text-xs mt-0.5 text-slate-400">{t("print.receiptPaySubtitle")}</p>
                </div>
              </div>

              <div className="text-end">
                <p className="text-[10px] sm:text-xs font-bold text-slate-400 mb-1">{t("print.receiptNumber")}</p>
                ...
                <p className="mt-1 text-[10px] sm:text-xs text-slate-500">
                  {new Date(purchasePayment.date).toLocaleDateString(locale === "ar" ? "ar-SA" : "en-US", { year: "numeric", month: "long", day: "numeric" })}
                </p>
              </div>
            </div>
            ...
            <div className="mb-4 sm:mb-6 rounded-xl sm:rounded-2xl border border-emerald-200 bg-emerald-50 py-6 sm:py-8 text-center">
              <p className="mb-1.5 sm:mb-2 text-[10px] sm:text-xs font-bold tracking-widest text-emerald-500">{t("print.amountPaid")}</p>
              <p className="text-3xl sm:text-5xl font-black text-emerald-700">
                {fmt(Number(purchasePayment.amount))}
              </p>
              <p className="mt-1 text-sm sm:text-base font-bold text-emerald-500">{t("print.sar")}</p>
            </div>

            {/* معلومات المورد */}
            <div className="mb-4 sm:mb-6 grid grid-cols-2 gap-3 sm:gap-4 rounded-xl sm:rounded-2xl bg-slate-50 p-3 sm:p-5">
              <div>
                <p className="mb-1 text-[10px] sm:text-xs font-bold text-slate-400">{t("print.supplier")}</p>
                <p className="text-sm sm:text-base font-bold text-slate-900">{supplier?.name || t("print.notDetermined")}</p>
                {supplier?.company && (
                  <p className="text-[10px] sm:text-xs text-slate-500 mt-0.5">{supplier.company}</p>
                )}
              </div>
              <div className="text-end">
                <p className="mb-1 text-[10px] sm:text-xs font-bold text-slate-400">{t("print.phone")}</p>
                <p className="text-sm sm:text-base font-bold text-slate-900" dir="ltr">{supplier?.phone || "—"}</p>
              </div>
            </div>
            ...
            {purchasePayment.notes && (
              <div className="mb-4 sm:mb-6 rounded-xl sm:rounded-2xl border border-slate-100 bg-slate-50 px-4 sm:px-5 py-3 sm:py-4">
                <p className="mb-1 text-[10px] sm:text-xs font-bold text-slate-400">{t("print.notes")}</p>
            ...
            <div className="mb-5 sm:mb-7 grid grid-cols-2 gap-4 sm:gap-6">
              <div className="text-center">
                ...
                <p className="mt-2 text-[10px] sm:text-xs text-slate-400">{t("print.payerSignature2")}</p>
              </div>
              <div className="text-center">
                <div className="h-8 sm:h-10 border-b border-slate-300" />
                <p className="mt-2 text-[10px] sm:text-xs text-slate-400">{t("print.supplierSignature")}</p>
              </div>
            </div>

            {/* Footer */}
            <div className="border-t border-slate-100 pt-4 sm:pt-5 text-center text-[10px] sm:text-xs text-slate-400">
              {t("print.footerPurchasePaymentNote")}
            </div>
          </div>
