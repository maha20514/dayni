...
export default function StoreOwnerPage() {
  ...
  const saveShopName = async () => {
    if (!newShopName.trim()) { toast.error(t("owner.shopNameRequired")); return; }
    setSavingName(true);
    try {
      ...
      const data = await res.json();
      if (!res.ok) { toast.error(data.error || t("owner.updateFailed")); return; }
      setShopName(newShopName);
      setIsEditingName(false);
      toast.success(t("owner.shopNameUpdated"));
      await refreshSession();
    } catch { toast.error(t("owner.connectionError")); } finally { setSavingName(false); }
  };

  const handleAvatarChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    const file = e.target.files?.[0];
    if (!file) return;
    if (file.size > 500 * 1024) { toast.error(t("owner.avatarTooLarge")); return; }
    ...
    reader.onload = async (event) => {
      ...
      try {
        ...
        setAvatar(base64);
        toast.success(t("owner.avatarUpdated"));
        await refreshSession();
      } catch (err: any) { toast.error(err.message || t("owner.avatarFailed")); } finally { setAvatarUploading(false); }
    };
    reader.readAsDataURL(file);
  };

  const handleChangePassword = async () => {
    if (!currentPassword || !newPassword || !confirmPassword) { toast.error(t("owner.allFieldsRequired")); return; }
    if (newPassword !== confirmPassword) { toast.error(t("owner.passwordMismatch")); return; }
    if (newPassword.length < 6) { toast.error(t("owner.passwordMin")); return; }
    ...
    try {
      ...
      const data = await res.json();
      if (!res.ok) throw new Error(data.error);
      toast.success(t("owner.passwordChanged"));
      setShowPasswordModal(false);
      setCurrentPassword(""); setNewPassword(""); setConfirmPassword("");
    } catch (err: any) { toast.error(err.message); } finally { setSavingPassword(false); }
  };

  const getPlanLabel   = () => plan === "pro" ? t("owner.planPro") : plan === "basic" ? t("owner.planBasic") : t("owner.planFree");
    ...
    return (
      <main dir={dir} className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
        <div className="rounded-2xl sm:rounded-3xl border border-slate-200 bg-white px-6 sm:px-8 py-5 sm:py-6 text-center shadow-xl shadow-slate-200/70">
          ...
          <p className="text-sm sm:text-lg font-bold text-slate-800">{t("owner.loadingTitle")}</p>
          <p className="mt-1 text-xs sm:text-sm text-slate-500">{t("owner.loadingDesc")}</p>
        </div>
      </main>
    );
  }
        ...
        <section className="mb-4 sm:mb-6 lg:mb-8 overflow-hidden rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-white shadow-lg shadow-slate-200/70">
          <div className="relative p-4 sm:p-6 lg:p-10">
            ...
            <div className="relative flex flex-col gap-3 sm:gap-4 lg:flex-row lg:items-end lg:justify-between">
              ...
              >
                {t("owner.backToDashboard")} <span className="ms-1 sm:ms-2">←</span>
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="grid gap-4 sm:gap-6 lg:gap-8 lg:grid-cols-[1.1fr_0.9fr]">
          ...
          <div className="rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-white p-4 sm:p-6 lg:p-8 shadow-lg shadow-slate-200/70">
            ...
            <div className="mb-5 sm:mb-6 lg:mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
              <div className="flex items-start gap-3 sm:gap-4 lg:gap-5">
                ...
                <div className="relative shrink-0">
                  ...
                  <label className={`absolute -bottom-2 -end-2 flex h-9 w-9 sm:h-10 sm:w-10 lg:h-11 lg:w-11 cursor-pointer items-center justify-center rounded-xl sm:rounded-2xl border border-slate-200 bg-white text-lg sm:text-xl shadow-lg transition hover:bg-slate-50 ${avatarUploading ? "opacity-50 cursor-wait" : ""}`}>
                    {avatarUploading
                ...
                <div className="flex-1 min-w-0">
                  {!isEditingName ? (
                    ...
                    <div className="space-y-2 sm:space-y-3">
                      <input
                        ...
                        placeholder={t("owner.shopNamePlaceholder")}
                      />
                  ...
                  <p className="mt-1.5 sm:mt-2 text-xs sm:text-sm lg:text-base font-semibold text-slate-500 truncate" dir="ltr">{email}</p>
            ...
            <div className="grid grid-cols-2 gap-2 sm:gap-3 lg:gap-4">
              {[
                { icon: "👥", label: t("owner.statCustomers"),  value: fmt(stats.totalCustomers),                         color: "blue"    },
                { icon: "💰", label: t("owner.statTotalDebt"),  value: `${fmt(stats.totalDebt)} ${t("owner.riyal")}`,     color: "red"     },
                { icon: "🧾", label: t("owner.statInvoices"),   value: fmt(stats.totalInvoices),                          color: "amber"   },
                { icon: "✅", label: t("owner.statPayments"),   value: fmt(stats.totalPayments),                          color: "emerald" },
              ].map(s => {
                const colorMap: any = {
          ...
          <div className="space-y-4 sm:space-y-5 lg:space-y-6">
            <div className="rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-white p-4 sm:p-6 lg:p-8 shadow-lg shadow-slate-200/70">
              <div className="mb-4 sm:mb-5 lg:mb-6">
                <h2 className="text-base sm:text-xl lg:text-2xl font-bold text-slate-950">{t("owner.accountSettings")}</h2>
                <p className="mt-0.5 sm:mt-1 text-xs sm:text-sm font-medium text-slate-500">
                  {t("owner.accountSettingsDesc")}
                </p>
              </div>
              ...
              <div className="space-y-2 sm:space-y-3">
                ...
                <button
                  ...
                  className="flex w-full items-center justify-between rounded-xl sm:rounded-2xl lg:rounded-3xl border border-slate-200 bg-slate-50 p-3 sm:p-4 lg:p-5 text-start transition hover:border-blue-200 hover:bg-blue-50/40"
                >
                  <div className="flex items-center gap-2.5 sm:gap-3 lg:gap-4">
                    ...
                    <div>
                      <p className="text-xs sm:text-sm lg:text-base font-bold text-slate-900">{t("owner.changePassword")}</p>
                      <p className="mt-0.5 hidden sm:block text-xs sm:text-sm font-medium text-slate-500">{t("owner.changePasswordDesc")}</p>
                    </div>
                  </div>
                ...
                <Link
                  href="/support"
                  className="flex w-full items-center justify-between rounded-xl sm:rounded-2xl lg:rounded-3xl border border-slate-200 bg-slate-50 p-3 sm:p-4 lg:p-5 text-start transition hover:border-emerald-200 hover:bg-emerald-50/40"
                >
                  <div className="flex items-center gap-2.5 sm:gap-3 lg:gap-4">
                    ...
                    <div>
                      <p className="text-xs sm:text-sm lg:text-base font-bold text-slate-900">{t("owner.support")}</p>
                ...
                {canExport ? (
                  <button
                    onClick={() => toast.info(t("owner.exportStarting"))}
                    className="flex w-full items-center justify-between rounded-xl sm:rounded-2xl lg:rounded-3xl border border-slate-200 bg-slate-50 p-3 sm:p-4 lg:p-5 text-start transition hover:border-emerald-200 hover:bg-emerald-50/40"
                  >
                    <div className="flex items-center gap-2.5 sm:gap-3 lg:gap-4">
                      ...
                      <div>
                        <p className="text-xs sm:text-sm lg:text-base font-bold text-slate-900">{t("owner.exportData")}</p>
                        <p className="mt-0.5 hidden sm:block text-xs sm:text-sm font-medium text-slate-500">{t("owner.exportDataDesc")}</p>
                      </div>
                    </div>
                    <span className="text-slate-400 text-sm">←</span>
                  </button>
                  ...
                  <div className="relative group">
                    <button
                      className="flex w-full items-center justify-between rounded-3xl border border-slate-200 bg-slate-50 p-5 text-start transition hover:border-amber-200 hover:bg-amber-50/40"
                    >
                      <div className="flex items-center gap-4">
                        <div className="relative flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-50 text-2xl">
                          📤
                          <div className="absolute -end-1 -top-1 text-sm">🔒</div>
                        </div>
                        <div>
                          <p className="font-bold text-slate-900">{t("owner.exportData")}</p>
                          <p className="mt-1 text-sm font-medium text-slate-500">{t("owner.exportDataDesc")}</p>
                        </div>
                      </div>
                    ...
                    {/* Tooltip */}
                    <div className="pointer-events-none absolute start-1/2 top-full z-50 mt-3 w-72 -translate-x-1/2 scale-95 rounded-3xl border border-slate-200 bg-white p-5 text-center opacity-0 shadow-2xl transition-all duration-200 group-hover:pointer-events-auto group-hover:scale-100 group-hover:opacity-100">
                      <p className="mb-1 text-lg font-black text-slate-900">🔒 {t("owner.proFeatureLocked")}</p>
                      <p className="mb-5 text-sm leading-7 text-slate-500">{t("owner.exportLockedDesc")}</p>
                      <Link
                        href="/pricing"
                      ...
                      <div className="absolute -top-2 start-1/2 h-4 w-4 -translate-x-1/2 rotate-45 border-s border-t border-slate-200 bg-white" />
                    </div>
                  </div>
                )}
                ...
                {plan !== "free" && (
                  <Link
                    ...
                    className="flex w-full items-center justify-between rounded-xl sm:rounded-2xl lg:rounded-3xl border border-slate-200 bg-slate-50 p-3 sm:p-4 lg:p-5 text-start transition hover:border-purple-200 hover:bg-purple-50/40"
                  >
                    <div className="flex items-center gap-2.5 sm:gap-3 lg:gap-4">
                      ...
                      <div>
                        <p className="text-xs sm:text-sm lg:text-base font-bold text-slate-900">{t("owner.billing")}</p>
                        <p className="mt-0.5 hidden sm:block text-xs sm:text-sm font-medium text-slate-500">{t("owner.billingDesc")}</p>
                      </div>
                    </div>
                    <span className="text-slate-400 text-sm">←</span>
                  </Link>
            ...
            <div className="overflow-hidden rounded-2xl sm:rounded-[2rem] border border-blue-200 bg-blue-50 p-4 sm:p-5 lg:p-6 shadow-sm">
              ...
              >
                {t("owner.viewPlans")} <span className="ms-1 sm:ms-2">←</span>
              </Link>
            </div>
          </div>
        </section>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/70 p-3 sm:p-4 backdrop-blur-sm">
          <div className="w-full max-w-sm sm:max-w-md rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-white p-5 sm:p-6 lg:p-8 shadow-2xl">
            <div className="mb-5 sm:mb-6 lg:mb-8 text-center">
              ...
              <h3 className="text-lg sm:text-xl lg:text-2xl font-bold text-slate-950">{t("owner.passwordModalTitle")}</h3>
              <p className="mt-1 sm:mt-2 text-xs sm:text-sm font-medium text-slate-500">{t("owner.passwordModalDesc")}</p>
            </div>

            <div className="space-y-3 sm:space-y-4">
              {[
                { placeholder: t("owner.currentPasswordPlaceholder"), value: currentPassword, setter: setCurrentPassword },
                { placeholder: t("owner.newPasswordPlaceholder"),     value: newPassword,     setter: setNewPassword     },
                { placeholder: t("owner.confirmPasswordPlaceholder"), value: confirmPassword, setter: setConfirmPassword  },
              ].map(f => (
                <input
                  key={f.placeholder}
                  type="password"
