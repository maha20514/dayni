...
export default function CollectionPage() {
  ...
  const chartData = {
    ...
    datasets: [
      { label: t("reportsCollection.invoicesLabel"), data: monthly.map((m) => m.inv), backgroundColor: "#ef4444", borderRadius: 6, borderSkipped: false },
      { label: t("reportsCollection.paymentsLabel"), data: monthly.map((m) => m.pay), backgroundColor: "#22c55e", borderRadius: 6, borderSkipped: false },
    ],
  };
  ...
  const chartOptions = {
    ...
    plugins: {
      ...
      tooltip: { rtl: dir === "rtl", callbacks: { label: (ctx: any) => ` ${ctx.dataset.label}: ${fmtM(Number(ctx.raw))}` } },
    },
    scales: {
    ...
    return (
      <main dir={dir} className="flex min-h-screen items-center justify-center bg-slate-50 px-4">
        <div className="rounded-2xl border border-slate-200 bg-white px-6 py-5 text-center shadow-xl">
          ...
          <p className="text-sm font-bold text-slate-800">{t("reportsCollection.loadingTitle")}</p>
        </div>
      </main>
    );
  }
  ...
  return (
    <main dir={dir} className="relative min-h-screen overflow-hidden bg-slate-50 py-6 sm:py-10 text-slate-900">
      ...
      <div className="absolute bottom-20 start-0 h-56 w-56 sm:h-72 sm:w-72 rounded-full bg-emerald-100/40 blur-3xl" />
      ...
      <div className="container relative z-10 mx-auto max-w-6xl px-3 sm:px-6">
          ...
          <div className="relative p-4 sm:p-8 md:p-10">
            ...
            <div className="relative flex flex-col gap-4 sm:gap-6 lg:flex-row lg:items-end lg:justify-between">
              <div>
                ...
                <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-semibold leading-tight text-slate-950">{t("reportsCollection.title")}</h1>
                <p className="mt-2 sm:mt-3 text-sm sm:text-base lg:text-lg leading-relaxed text-slate-600">
                  {t("reportsCollection.subtitle")}
                </p>
              </div>
              <Link href="/reports" className="inline-flex items-center justify-center rounded-xl sm:rounded-2xl bg-blue-600 px-4 sm:px-6 py-3 sm:py-4 text-xs sm:text-base font-bold text-white shadow-lg sm:shadow-xl shadow-blue-500/25 transition-all hover:-translate-y-1 hover:bg-blue-700 whitespace-nowrap">
                {t("reportsCollection.backToReports")} <span className="ms-1 sm:ms-2">←</span>
              </Link>
            </div>
          </div>
        </section>
        ...
        <section className="mb-5 sm:mb-8 grid gap-3 sm:gap-5 grid-cols-2 lg:grid-cols-4">
          {[
            { label: t("reportsCollection.statInvoices"),  value: fmtM(stats.totalInvoices),  icon: "🧾", bg: "bg-red-50",     text: "text-red-700",     border: "border-red-200" },
            { label: t("reportsCollection.statPaid"),      value: fmtM(stats.totalPaid),      icon: "✅", bg: "bg-emerald-50", text: "text-emerald-700", border: "border-emerald-200" },
            { label: t("reportsCollection.statRemaining"), value: fmtM(stats.totalRemaining), icon: "⏳", bg: "bg-amber-50",   text: "text-amber-700",   border: "border-amber-200" },
            { label: t("reportsCollection.statDso"),       value: `${stats.dso} ${t("reportsCollection.days")}`, icon: "📊", bg: "bg-blue-50", text: "text-blue-700", border: "border-blue-200" },
          ].map((c) => (
        ...
        <section className="mb-5 sm:mb-8 rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-white p-4 sm:p-6 shadow-lg sm:shadow-xl shadow-slate-200/70">
          <div className="mb-3 sm:mb-4 flex items-center justify-between">
            <div>
              <h2 className="text-base sm:text-xl font-bold text-slate-950">{t("reportsCollection.rateTitle")}</h2>
              <p className="mt-0.5 sm:mt-1 text-xs sm:text-sm text-slate-500 hidden sm:block">{t("reportsCollection.rateDesc")}</p>
            </div>
          ...
          <div className="mt-2 sm:mt-3 flex justify-between text-xs sm:text-sm font-semibold text-slate-400">
            <span>{t("reports.paidLabel")}: {fmtM(stats.totalPaid)}</span>
            <span>{t("reports.remainingLabel")}: {fmtM(stats.totalRemaining)}</span>
          </div>
        </section>
        ...
        <section className="mb-5 sm:mb-8 rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-white p-4 sm:p-6 shadow-lg sm:shadow-xl shadow-slate-200/70">
          <div className="mb-4 sm:mb-6">
            <h2 className="text-base sm:text-2xl font-bold text-slate-950">{t("reportsCollection.chartTitle")}</h2>
            <p className="mt-0.5 sm:mt-1 text-xs sm:text-sm text-slate-500 hidden sm:block">{t("reportsCollection.chartDesc")}</p>
          </div>
        ...
        <section className="rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-white shadow-lg sm:shadow-xl shadow-slate-200/70">
          <div className="border-b border-slate-100 px-4 sm:px-6 py-3 sm:py-4">
            <h2 className="text-sm sm:text-base font-bold text-slate-900">{t("reportsCollection.monthlyTableTitle")}</h2>
          </div>
