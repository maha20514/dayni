...
type Section = { title: string; body?: string; intro?: string; items?: string[]; outro?: string; isDataTable?: boolean };
...
export default function PrivacyPage() {
  ...
  const sections = tRaw("privacyPage.sections") as unknown as Section[];
  ...
  return (
    <main dir={dir} className="relative min-h-screen overflow-hidden bg-slate-50 py-10 sm:py-16 text-slate-900">
      <div className="absolute end-0 top-20 h-72 w-72 rounded-full bg-emerald-100/50 blur-3xl pointer-events-none" />
      <div className="absolute bottom-20 start-0 h-72 w-72 rounded-full bg-blue-100/40 blur-3xl pointer-events-none" />
      ...
      <div className="container relative z-10 mx-auto max-w-4xl px-4 sm:px-6">
        ...
        <section className="mb-8 overflow-hidden rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-white shadow-xl shadow-slate-200/70">
          <div className="relative p-6 sm:p-10">
            <div className="absolute start-0 top-0 h-28 w-28 rounded-ee-[3rem] bg-emerald-50 hidden sm:block" />
            <div className="absolute bottom-0 end-0 h-28 w-28 rounded-ss-[3rem] bg-teal-50 hidden sm:block" />
        ...
        <div className="space-y-6">
          {sectionsArr.map((section) => (
            <PolicySection key={section.title} title={section.title}>
              {section.isDataTable ? (
                <>
                  <p className="mb-3">{t("privacyPage.intro")}</p>
                  <div className="space-y-3">
                    <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                      <p className="font-bold text-slate-800">{t("privacyPage.dataAccountTitle")}</p>
                      <p className="mt-1 text-slate-600">{t("privacyPage.dataAccountBody")}</p>
                    </div>
                    <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                      <p className="font-bold text-slate-800">{t("privacyPage.dataUsageTitle")}</p>
                      <p className="mt-1 text-slate-600">{t("privacyPage.dataUsageBody")}</p>
                    </div>
                    <div className="rounded-xl border border-slate-100 bg-slate-50 p-4">
                      <p className="font-bold text-slate-800">{t("privacyPage.dataTechTitle")}</p>
                      <p className="mt-1 text-slate-600">{t("privacyPage.dataTechBody")}</p>
                    </div>
                  </div>
                </>
              ) : (
                    ...
                    <ul className="mt-2 space-y-2 text-slate-600">
                      {section.items.map((item) => (
                        <li key={item} className="flex items-start gap-2">
                          <span className="mt-1 text-emerald-500">✓</span>{item}
                        </li>
                      ))}
                    </ul>
                  )}
