...
function ResetForm() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    } catch { setError(t("resetPassword.connectionError")); } finally { setLoading(false); }
  };
    ...
    <main dir={dir} className="relative flex min-h-screen items-center justify-center overflow-hidden bg-slate-50 px-3 py-6 sm:px-6 sm:py-10">
      ...
      <div className="relative z-10 w-full max-w-md rounded-2xl sm:rounded-[2rem] border border-slate-200 bg-white p-5 sm:p-8 lg:p-10 shadow-2xl">
        <div className="mb-5 sm:mb-8 text-center">
          ...
          <span className="mb-2 sm:mb-3 inline-flex rounded-full bg-blue-50 px-3 sm:px-5 py-1.5 sm:py-2 text-xs sm:text-sm font-bold text-blue-700">{t("resetPassword.badge")}</span>
          <h1 className="mt-2 text-xl sm:text-3xl font-bold text-slate-950">{t("resetPassword.title")}</h1>
        </div>
        ...
        {success ? (
          <div className="rounded-xl sm:rounded-2xl border border-emerald-200 bg-emerald-50 p-4 sm:p-6 text-center">
            ...
            <p className="text-sm sm:text-base font-bold text-emerald-800">{t("resetPassword.successTitle")}</p>
            <p className="mt-1 text-xs sm:text-sm text-emerald-700">{t("resetPassword.successDesc")}</p>
          </div>
        ) : (
          <form onSubmit={handleSubmit} className="space-y-3 sm:space-y-5">
            {[
              { label: t("resetPassword.newPassword"), value: password, setter: setPassword },
              { label: t("resetPassword.confirmPassword"), value: confirm, setter: setConfirm },
            ].map((f) => (
              <div key={f.label}>
        ...
        <div className="mt-4 sm:mt-6 text-center">
          <Link href="/login" className="text-xs sm:text-sm font-bold text-blue-600 hover:text-blue-700">{t("resetPassword.backToLogin")}</Link>
        </div>
      </div>
    </main>
  );
